...
export async function GET(request: Request) {
  ...
  const { data, error } = await service.from('budgets').select('*').eq('camp_id', campId)
...
export async function POST(request: Request) {
  ...
  const { camp_id, category, planned_amount } = await request.json()
  try {
  ...
  const { data, error } = await service
    .from('budgets')
    .upsert({ camp_id, category, planned_amount }, { onConflict: 'camp_id,category' })
    .select()
    .single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
